 ...
 * @author LENOVO
 */
public class RegistrarQuemaQuimica extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Long id = co1.RegistroQuemaQumica(fecha, num_lote, cantidad_mano_obra, valor_mano_obra, valor_total_mano_obra, costo_total);
                for (int i = 0; i < tipo_insumo.length; i++) {
                    System.out.println("imprimio3");
                    if (co1.RegistrarInsumosQuemaQuimica(id, tipo_insumo[i],nombre_insumo[i], cantidad[i], unidad_insumo[i], valor_unitario[i].replace(".", ""), valor_total[i].replace(".", ""))) {
            ...
            try {
                if (resultado) {
                    ...
                    response.sendRedirect("QuemaQuimica.jsp");
                } else {
